...
public sealed partial class TeamsApp
{
    ...
    {
        // _nomd: escapes markdown control characters to disable markdown
        Expression.Functions.Add("_nomd", NoMarkdown);
        // _jsonPrettify: jsonStringify() rendered indented, with null values omitted
        Expression.Functions.Add("_jsonPrettify", JsonPrettify);
        // _colorUri: data uri with a one-pixel image of the requested color
        Expression.Functions.Add("_colorUri", ColorUri);
    }

    private static object NoMarkdown(IReadOnlyList<object> args)
    {
        var input = args[0];
    ...
    private static object JsonPrettify(IReadOnlyList<object> args)
    {
    ...
    private static object ColorUri(IReadOnlyList<object> args)
    {
